...
contract('Bidding Ring tests', async (accounts) => {
    it("Bidding Ring test 2", async () => {
        ...
        let bid2 = web3.eth.abi.encodeParameters(['uint', 'uint'], [2, 10]);
        ...
        await ring.reveal.sendTransaction(10, {from: accounts[3], gas: 3000000, value:2});
        ...
        await auction.bid.sendTransaction(web3.utils.keccak256(bid4, {encoding: "hex"}), {from: accounts[4], gas: 3000000});

        // let bid5 = web3.eth.abi.encodeParameters(['uint', 'uint'], [3, 10]);
        // await auction.bid.sendTransaction(web3.utils.keccak256(bid5, {encoding: "hex"}), {from: accounts[3], gas: 3000000});

        let bid6 = web3.eth.abi.encodeParameters(['uint', 'uint'], [3, 10]);
        await auction.bid.sendTransaction(web3.utils.keccak256(bid6, {encoding: "hex"}), {from: accounts[5], gas: 3000000});

        await auction.reveal.sendTransaction(10, {from: accounts[4], gas: 3000000, value:1});
        await auction.reveal.sendTransaction(10, {from: accounts[5], gas: 3000000, value:3});
        // await auction.reveal.sendTransaction(10, {from: accounts[6], gas: 3000000, value:3});
        await ring.revealToAuction.sendTransaction({from: accounts[2], gas: 3000000});
        let bal = await auction.getNumRevealed.call();
        assert(bal.valueOf(), 3, "Not Matching");
        await auction.claimBalance.sendTransaction({from: accounts[5]});

        let ans = await auction.getBid.call();
        assert(ans[0].valueOf(), accounts[5], "Incorrect Highest Bidder");
