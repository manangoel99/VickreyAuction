...
contract('Bidding Ring tests', async (accounts) => {
    it("Bidding Ring test 3", async () => {
        let auction = await VickreyAuction.deployed();
        let ring = await BiddingRing.deployed();

        let bid = web3.eth.abi.encodeParameters(['uint', 'uint'], [2, 10]);
        ...
        let bid2 = web3.eth.abi.encodeParameters(['uint', 'uint'], [3, 10]);
        await ring.bid.sendTransaction(web3.utils.keccak256(bid2, {encoding: "hex"}), {from: accounts[3], gas: 3000000});

        await ring.reveal.sendTransaction(10, {from: accounts[1], gas: 3000000, value:2});
        await ring.reveal.sendTransaction(10, {from: accounts[3], gas: 3000000, value:3});
        ...
        let bid4 = web3.eth.abi.encodeParameters(['uint', 'uint'], [2, 10]);
        await auction.bid.sendTransaction(web3.utils.keccak256(bid4, {encoding: "hex"}), {from: accounts[4], gas: 3000000});


        let bid6 = web3.eth.abi.encodeParameters(['uint', 'uint'], [4, 10]);
        ...
        await ring.revealToAuction.sendTransaction({from: accounts[3], gas: 3000000});

        let bal = await auction.getNumRevealed.call();
        assert(bal.valueOf(), 2, "Not Matching");
        await auction.claimBalance.sendTransaction({from: accounts[3]});
